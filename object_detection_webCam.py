...
def detection(detection_graph, category_index, score, expand):
    ...
    with detection_graph.as_default():
        with tf.Session(graph=detection_graph,config=config) as sess:
            ...
            print('WEB Cam as input')
            cap = cv2.VideoCapture("/dev/video1")
            ret, image = cap.read()
            height, width, channels = image.shape
            print 'Image weight, height and number of channels: ' + str(height) + str(width) + str(channels)
    ...
    print('> [INFO] Max. FPS: {:.2f}'.format(fps.max()))
    #print('> [INFO] Min. FPS: {:.2f}'.min(fps.fps()))
    #print('> [INFO] Median. FPS: {:.2f}'.median(fps.fps()))
    #print('> [INFO] Mode. FPS: {:.2f}'.max(fps.fps()))
